          ...
          <div className="flex items-center">
            ...
            >
              <div className="flex items-center space-x-2">
                <span className="text-lg md:text-xl 2xl:text-2xl font-bold gradient-text">{conferenceInfo.shortTitle}</span>
              </div>
            </Link>
          </div>

          {/* Desktop Navigation */}
          <nav className="hidden md:flex items-center space-x-2 2xl:space-x-4">
            {navigationItems.map((item) => (
              <NavLink
                key={item.name}
                to={item.href}
                className={`px-2.5 py-1.5 2xl:px-3 text-sm lg:text-base 2xl:text-lg font-medium rounded-md cursor-pointer transition-colors duration-300 ${
                  isActive(item.href)
            ...
            >
              {theme === 'light' ? <Moon size={20} className="2xl:w-6 2xl:h-6" /> : <Sun size={20} className="2xl:w-6 2xl:h-6" />}
            </button>
          </nav>
